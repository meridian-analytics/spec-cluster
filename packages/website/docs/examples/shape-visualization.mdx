...
import * as Demo from "@spec-cluster/example-shape-visualization"
...
These selected entities can be further manipulated via the custom <u>Multi-select Editor</u>.
...
- **renderMode**: Set to `dot` to display spectrograms as spheres (default shape). The `image` mode is covered in the [Spec visualization example](./spec-visualization).
