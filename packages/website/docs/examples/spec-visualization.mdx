...
- Once clicked, the spectrogram's details are displayed through a custom UI component, [<u>FocusModal</u>](#focus-modal).
...
- **renderMode:** Set to `image` for this demo to render spectrograms visually as images. The `dot` mode is covered in the [Shape visualization example](shape-visualization).
